...
- (CGSize)computeTextSize:(NSString *)text
{
    ...
    CGSize boundSize = CGSizeMake(500, 40);
    // CGSize stringSize = [_text sizeWithFont:_font constrainedToSize:boundSize lineBreakMode:UILineBreakModeWordWrap];
    CGSize stringSize = [_text sizeWithFont:_font constrainedToSize:boundSize lineBreakMode:NSLineBreakByWordWrapping];
    return stringSize;
}
...
- (id)initWithFrame:(CGRect)frame BackGoundColor:(UIColor *)backgoundColor Text:(NSString *)text TextColor:(UIColor *)tmpTextColor textFont:(CGFloat)fontFloat
{
    ...
    if (self)
    {
        ...
        _frame = frame;
        //默认的字体大小
        _font = [UIFont systemFontOfSize:fontFloat];
